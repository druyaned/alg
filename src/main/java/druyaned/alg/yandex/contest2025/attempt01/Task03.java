...
public class Task03 {
    ...
    private static void solve(Sio sio) throws IOException {
        ...
        int idCount = 0;
        // [0] -> timestamp, [1] -> pos
        long[][] idToEntry = new long[n][];
        int[] result = new int[5];
        int size;
        for (int i = 0; i < n; i++) {
            String type = sio.readToken();
            if (type.equals("TAXI")) {
                long timestamp = sio.readLong();
                int id = sio.readInt();
                int pos = sio.readInt();
                if (idToEntry[id] == null)
                    ids[idCount++] = id;
                idToEntry[id] = new long[] { timestamp, pos };
            }
            if (type.equals("ORDER")) {
                long orderTimestamp = sio.readLong();
                sio.readInt(); // orderId
                int orderPos = sio.readInt();
                int orderTime = sio.readInt();
                Arrays.fill(result, 0);
                size = 0;
                for (int j = 0; j < idCount && size < 5; j++) {
                    int id = ids[j];
                    long timestamp = idToEntry[id][0];
                    int pos = (int)idToEntry[id][1];
                    long dl = (orderTimestamp - timestamp) * s;
                    long posDiff = orderPos < pos
                            ? l - pos + orderPos
                            : orderPos - pos;
                    int len = dl < posDiff ? (int)(posDiff - dl) : 0;
                    int time = len / s;
                    if (time <= orderTime)
                        result[size++] = id;
                }
                if (size == 0) {
                    sio.writeln(Integer.toString(-1));
                } else {
                    sio.write(Integer.toString(result[0]));
